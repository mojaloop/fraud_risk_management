...
const handleQuoteMessage = async (message: kafka.Message, topic: string) => {
  ...
  if (!sourceAccountPINDate) {
    publish(
      topic,
      `Transaction: ${txID} doesn't have PIN change information`,
      false,
    );
    return;
  }
  ...
  const hours =
    Math.abs(Date.now() - new Date(jMessage.SourceAccountPINDate).valueOf()) /
    3600000;
  if (hours < 72) {
    ...
    publish(
      topic,
      `Transaction: ${txID} has not recently changed their PIN`,
      false,
    );
  }
};
