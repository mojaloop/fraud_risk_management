...
const handleQuoteMessage = async (
  ...
  try {
    ...
    try {
      ...
      log(
        `Error while handling New payee transfer ${TransactionID}, with message: \r\n${error}`,
        topic,
      );
    }
    try {
      scores.rule48 = rules.handleLargeTransactionPayer(
        transfer,
        sourceHistoricalSendData,
      );
    } catch (error) {
      log(
        `Error while handling Large Transaction Payer ${TransactionID}, with message: \r\n${error}`,
        topic,
      );
    }
    try {
      ...
      log(
        `Error while handling Cash Withdraw ${TransactionID}, with message: \r\n${error}`,
        topic,
      );
    }
