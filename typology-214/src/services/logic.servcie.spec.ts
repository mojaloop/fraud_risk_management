  ...
  describe('Handle Transaction', () => {
    it('should handle successful request, with a unmatched number', async () => {
      ...
      expect(result).toEqual(
        `[account-muling][Result][FALSE] Payer and Payee contact number is not the same for transaction ID: ${expectedReq.TransactionID}`,
      );
    });
    ...
    it('should handle successful request, with a matched number', async () => {
      ...
      expect(result).toEqual(
        `[account-muling][Result][TRUE] Payer and Payee contact number is the same for transaction ID: ${expectedReq.TransactionID}`,
      );
    });
  });
});
