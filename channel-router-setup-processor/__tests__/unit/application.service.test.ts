...
describe('Application Service', () => {
  ...
  describe('Get Online', () => {
    it('should handle a online get request', async () => {
      ...
      const loggerSpy = jest.spyOn(LoggerService, 'log').mockImplementation(() => Promise.resolve());

      await service.getOnline(ctx as ParameterizedContext<any, IRouterParamContext<any, {}>, any>);
  ...
  describe('Execute', () => {
    ...
    beforeEach(() => {
      ...
      logSpy = jest.spyOn(LoggerService, 'log').mockImplementation(() => Promise.resolve());

      errorSpy = jest.spyOn(LoggerService, 'error').mockImplementation(() => Promise.resolve());

      logicExecuteSpy = jest.spyOn(service.logicService, 'handleTransaction').mockImplementation(() => Promise.resolve(expectedResult));
    });
    ...
    it('should successfully process valid request', async () => {
      ...
      expect(logicExecuteSpy).toHaveBeenCalledWith(expect.objectContaining(mockRequest));
    ...
    it('should handle a error thrown when request is parsed', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(failedMessage, thrownError, 'ApplicationService');
