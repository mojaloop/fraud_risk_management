...
const getTestRegex = (msg: string, type: 'TRACE' | 'INFO' | 'ERROR' | 'WARN', operation = '') =>
  new RegExp(
...
describe('Logger Service', () => {
  ...
  describe('Trace', () => {
    it('should trace a message with defaults', async () => {
      ...
      expect(measuredArgs).toMatch(getTestRegex(expectedMessage, 'TRACE'));
    });

    it('should trace a message with defined operation', async () => {
      const expectedMessage = 'ExpectedLogMessage';
      const operation = 'TestService';
      await LoggerService.trace(expectedMessage, operation);
      const measuredArgs = consoleTraceSpy.mock.calls[0][0];
      expect(measuredArgs).toMatch(getTestRegex(expectedMessage, 'TRACE', operation));
    });
  });
  ...
  describe('Log', () => {
    ...
    it('should log a message with defined operation', async () => {
      ...
      const measuredArgs = consoleLoggerSpy.mock.calls[0][0];
      expect(measuredArgs).toMatch(getTestRegex(expectedMessage, 'INFO', operation));
    });
  });
  ...
  describe('Warn', () => {
    ...
    it('should log a message with defined operation', async () => {
      ...
      const measuredArgs = consoleWarnSpy.mock.calls[0][0];
      expect(measuredArgs).toMatch(getTestRegex(expectedMessage, 'WARN', operation));
    });
  });
  ...
  describe('Error', () => {
    ...
    it('should log a string message with defined operation', async () => {
      ...
      const measuredArgs = consoleErrorSpy.mock.calls[0][0];
      expect(measuredArgs).toMatch(getTestRegex(expectedMessage, 'ERROR', operation));
    });
    ...
    it('should error a error object message with defaults', async () => {
      ...
      const messageForRegex = fixErrorStackRegex(expectedMessage.stack as string);
    ...
    it('should error a error object message with child error and operation', async () => {
      ...
      expect(expectedMessage.stack).toBeTruthy();
      expect(measuredArgs).toMatch(getTestRegex(messageForRegex, 'ERROR', operation));
    });
  });
});
