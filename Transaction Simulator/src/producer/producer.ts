...
export class Producer {
  ...
  Start() {
    ...
    try {
      ...
      const kafkaHost = AppConfiguration.Setting().kafkaEndpoint;
      let ready = false;
      ...
      producer.on('ready', async function () {
        ...
        // Start watcher
        const watcher = new FileWatcher('./watch', './done');
        ...
        watcher.on('process', function process(file) {
          ...
          fs.readFile(watchFile, async function (error: any, data: any) {
            if (error)
              console.log(
                'Error while reading contents of file with message: ' + error,
              );
            else if (data) await chat.sendMessage(data, producer);
            else console.log('No contents in the file.');
      ...
      producer.on('error', function (err) {
        console.log('Error received from server with details: ' + err);
      });
