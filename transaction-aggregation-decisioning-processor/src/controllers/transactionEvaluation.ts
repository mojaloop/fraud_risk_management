...
const sendRequest = (config: IConfig, requestBody: requestBodyType) => {
...
const scoreTransaction = async (ctx: Context): Promise<Context> => {
  try {
    ...
    if (resultsArray.length === typologiesNeeded.length) {
      ...
      sendRequest(configuration, requestBody);
...
export { scoreTransaction, testResult };
