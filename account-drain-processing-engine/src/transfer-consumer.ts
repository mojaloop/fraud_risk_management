...
const handleQuoteMessage = async (
  ...
  await publish(topic, `[${isAccountDrain}] Transaction: ${TransactionID} from ${ILPSourceAccountAddress} to ${ILPDestinationAccountAddress} is ${(isAccountDrain) ? 'bigger' : 'not bigger'} than the account balance`);
};
